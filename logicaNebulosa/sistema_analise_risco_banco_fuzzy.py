...
risco = ctrl.Consequent(np.arange(0, 11, 1), 'risco', defuzzify_method='mom')

credito['Ruim'] = fuzz.trapmf(credito.universe, [0, 0, 2, 3])
credito['Regular'] = fuzz.trapmf(credito.universe, [2, 3, 6, 6])
credito['Bom'] = fuzz.trapmf(credito.universe, [6, 7, 8, 9])
credito['Excelente'] = fuzz.trapmf(credito.universe, [8, 9, 10, 10])
...
rule4 = ctrl.Rule(credito['Regular'] & divida['Moderada'] & ~renda['Baixa'], risco['Médio']) # SE o histórico de crédito é "Regular" E a dívida atual é "Moderada", o risco é "Moderado". A menos que a renda seja baixa
...
banco.input['credito'] = 3
banco.input['renda'] = 2
banco.input['divida'] = 5
